...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable
{
			...
			PreparedStatement statement = con.prepareStatement("SELECT clanId FROM characters WHERE charId=?"))
		{
			...
			try (ResultSet rs = statement.executeQuery())
			{
				...
				if (answer == 0)
				{
					...
					{
						try (PreparedStatement statement2 = con.prepareStatement("UPDATE characters SET deletetime=? WHERE charId=?"))
						{
							statement2.setLong(1, System.currentTimeMillis() + (Config.DELETE_DAYS * 86400000L)); // 24*60*60*1000 = 86400000
							statement2.setInt(2, objid);
							statement2.execute();
						}
					}
	...
	public static void deleteCharByObjId(int objid)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_friends WHERE charId=? OR friendId=?"))
			{
				statement.setInt(1, objid);
			...
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_hennas WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_macroses WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_quests WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_quest_global_data WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.executeUpdate();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_recipebook WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			...
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_skills WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			...
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_subclasses WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			...
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM seven_signs WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			...
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM item_attributes WHERE itemId IN (SELECT object_id FROM items WHERE items.owner_id=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			...
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_raid_points WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_reco_bonus WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_instance_time WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM characters WHERE charId=?"))
			{
				statement.setInt(1, objid);
				statement.execute();
			}
