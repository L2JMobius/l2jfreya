		...
		try
		{
			...
			else if (_command.startsWith("npc_"))
			{
				...
				try
				{
					...
					else if ((object instanceof L2Npc) && (endOfId > 0) && activeChar.isInsideRadius(object, Config.INTERACTION_DISTANCE_NPC, false, false))
					{
			...
			else if (_command.startsWith("summon_"))
			{
				...
				try
				{
					...
					if ((object instanceof L2MerchantSummonInstance) && (endOfId > 0) && activeChar.isInsideRadius(object, Config.INTERACTION_DISTANCE_NPC, false, false))
					{
			...
			// TODO: OTHERS COMMANDS IN CB LIKE.. DRESSME, TELEPORT,,,
			else if (_command.startsWith("_bbs") || _command.startsWith("bbs") || _command.startsWith("_friend") || _command.startsWith("_maillist_0_1_0_"))
			{
				...
				{
					activeChar.sendMessage(Config.SERVER_NAME + ": Community Board is disabled.");
				}
			}
			...
			else if (_command.startsWith("_playerhelp"))
			{
				L2PcInstance player = getClient().getActiveChar();
				PlayerVariables vars = player.getVariables();
				
				if (vars.getBoolean("@PLAYER_HELP") == false)
				{
					vars.set("@PLAYER_HELP", true);
					Announcements.getInstance().announceToAll(player.getName() + " call SOS with killing this monster: " + player.getTarget().getName());
					ThreadPoolManager.getInstance().scheduleGeneral(() ->
					{
						player.getVariables().set("@PLAYER_HELP", false);
						player.sendMessage("System cleaned your time for announce help message.");
					}, 600000);
					
					player.sendMessage("Next announce available in 10 minutes.");
				}
				else
				{
					player.sendMessage("You must wait 10 minutes to next announce help.");
				}
			}
			else if (_command.startsWith("_npc_droplist"))
			{
				L2PcInstance player = getClient().getActiveChar();
				
				NpcHtmlMessage html = new NpcHtmlMessage(0);
				int hpMul = Math.round((float) (((L2Character) player.getTarget()).getStat().calcStat(Stats.MAX_HP, 1, (L2Character) player.getTarget(), null) / BaseStats.CON.calcBonus((L2Character) player.getTarget())));
				if (hpMul == 0)
				{
					hpMul = 1;
				}
				final StringBuilder html1 = StringUtil.startAppend(1000, "<html><body>" + "<br1><center><font name=\"hs9\" color=\"LEVEL\">[ " + player.getTarget().getName() + " ]</font></center><br1>");
				
				if (!((L2Npc) player.getTarget()).getTemplate().getDropData().isEmpty())
				{
					StringUtil.append(html1, "<center><br1><font color=\"9933FF\">Spoil</font> - <font color=\"cc9900\">Drop</font> - <font color=\"ff00ff\">Quest</font></center>" + "<table width=\"270\">");
					for (L2DropCategory cat : ((L2Npc) player.getTarget()).getTemplate().getDropData())
					{
						for (L2DropData drop : cat.getAllDrops())
						{
							...
							DecimalFormat chanceFormat = new DecimalFormat("0.00##");
							
							String getChanceFinal = chanceFormat.format(((drop.getChance() * Config.RATE_DROP_ITEMS) * 0.0001)); // if chance is too low, show only 0.01%
							
							StringUtil.append(html1, "<tr>"//
								+ "<td align=left><img src=" + item.getIcon() + " height=32 width=32></td>"//
								+ "<td align=left><font color=" + color + ">" + item.getName() + "</font><br1><font color=\"66FF66\">Chance:</font> " + getChanceFinal + "% <font color=\"66FF66\">Min:</font> " + min + " <font color=\"66FF66\">Max:</font> " + max + "</td></tr>" + "<tr><td><img src=L2UI.SquareGray width=32 height=2></td><td><img src=L2UI.SquareGray width=238 height=2></td></tr>");//
						}
					}
					html1.append("</table>");
				}
