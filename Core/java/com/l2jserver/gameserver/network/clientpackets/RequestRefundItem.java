...
public final class RequestRefundItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		) || !((target instanceof L2MerchantInstance) || (target instanceof L2MerchantSummonInstance)) || (player.getInstanceId() != target.getInstanceId()) || !player.isInsideRadius(target, Config.INTERACTION_DISTANCE_NPC, true, false))) // Distance is too far
		{
			sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		if (merchant != null)
		{
			...
			{
				lists = BuyListTable.getInstance().getBuyListByNpcId(((L2MerchantInstance) merchant).getNpcId());
				taxRate = ((L2MerchantInstance) merchant).getMpc().getTotalTaxRate();
			}
			else
			{
				lists = BuyListTable.getInstance().getBuyListByNpcId(((L2MerchantSummonInstance) merchant).getNpcId());
				taxRate = 50;
			}
			...
			{
				list = BuyListTable.getInstance().getBuyList(_listId);
			}
		}
		else
		{
