...
public final class RequestJoinPledge extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (target.isInOlympiadMode() || activeChar.isInOlympiadMode())
		{
			if ((target.isInOlympiadMode() != activeChar.isInOlympiadMode()) || (target.getOlympiadGameId() != activeChar.getOlympiadGameId()) || (target.getOlympiadSide() != activeChar.getOlympiadSide()))
			{
				_log.info("Olympiad: Warning player " + activeChar.getName() + " is trying to request join pledge to player " + target + " in olympiad match");
				activeChar.sendMessage("Dont try to request join when you in olympiad. Olympiad system inform GM/Admin stuff.");
				return;
			}
		}
