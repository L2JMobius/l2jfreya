...
public class HtmCache
{
	...
	private final Map<Integer, String> _cache = Config.LAZY_CACHE ? new ConcurrentHashMap<>() : new HashMap<>();
	...
	public void reloadPath(String path)
	{
		...
		_log.info("Cache[HTML]: Reloaded specified " + path + " path.");
	}
	...
	public String getHtmForce(L2PcInstance activeChar, String path)
	{
		String content = getHtm(activeChar, path);
		if (content == null)
		{
	...
	public String getHtm(L2PcInstance activeChar, String path)
	{
		String newPath = null;
		String content;
		String prefix = activeChar == null ? null : activeChar.getHtmlPrefix();
		if ((prefix != null) && !prefix.isEmpty())
		{
			...
			if (content != null)
			{
				if ((activeChar != null) && activeChar.isGM())
				{
					...
					{
						activeChar.sendMessage("Html Path: " + getClass() + " -  " + newPath);
					}
				}
				return content;
			}
		...
		if ((activeChar != null) && activeChar.isGM())
		{
			if (Config.ENABLE_HTML_PATH)
			{
				activeChar.sendMessage("Html Path: " + getClass() + " -  " + path);
			}
		}
		return content;
	}
