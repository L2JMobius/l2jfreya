...
public class MessageBoxBBSManager extends BaseBBSManager
{
	...
	{
		String html = HtmCache.getInstance().getHtm(activeChar, "data/html/CommunityBoard/message_box.htm");
		...
		else if (event[1].equals("view"))
		{
			...
			html = HtmCache.getInstance().getHtm(activeChar, "data/html/CommunityBoard/message.htm");
			try (Connection con = L2DatabaseFactory.getInstance().getConnection();
