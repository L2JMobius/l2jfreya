	...
	public static void storeOffliners()
	{
			...
			PreparedStatement stm_items = con.prepareStatement(SAVE_ITEMS))
		{
			...
			for (L2PcInstance pc : L2World.getInstance().getPlayers())
			{
				...
				{
					_log.info("Error while saving offline trader: " + pc.getObjectId());
				}
			}
		...
		{
			_log.info("Error while saving offline traders: " + e);
		}
	}
	...
	public static void restoreOfflineTraders()
	{
			...
			ResultSet rs = stm.executeQuery(LOAD_OFFLINE_STATUS))
		{
			while (rs.next())
			{
						...
						try (ResultSet items = stm_items.executeQuery())
						{
							switch (type)
							{
								...
								case L2PcInstance.STORE_PRIVATE_PACKAGE_SELL:
									...
									player.getSellList().setPackaged(type == L2PcInstance.STORE_PRIVATE_MANUFACTURE);
									break;
								case L2PcInstance.STORE_PRIVATE_MANUFACTURE:
				...
				{
					_log.info("Error loading trader: " + player);
					if (player != null)
					{
						player.deleteMe();
					}
		...
		{
			_log.info("Error while loading offline traders: " + e);
		}
	}
