...
public class EnchantGroupsTable
{
	...
	private Map<Integer, L2EnchantSkillGroup> _enchantSkillGroups; // enchant skill group
	...
	private void load()
	{
		...
		{
			_enchantSkillGroups = new HashMap<>();
			_enchantSkillTrees = new HashMap<>();
			int prevGroupId = -1;
			
			try (ResultSet enchantGroups = statement.executeQuery())
			{
		...
		_log.info("Loaded " + count + " enchant groups.");
	}
