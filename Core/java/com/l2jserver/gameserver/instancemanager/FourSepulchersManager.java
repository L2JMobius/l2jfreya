...
import com.l2jserver.scripts.quests.Q00620_FourGoblets;
...
public class FourSepulchersManager
{
	...
	protected void timeSelector()
	{
		...
		// check
		{
			...
			_log.info("Four Sepulchers: Beginning in Entry time");
		}
		...
		// check
		{
			...
			_log.info("Four Sepulchers: Beginning in WarmUp time");
		}
		...
		// check
		{
			...
			_log.info("Four Sepulchers: Beginning in Attack time");
		}
		else
		...
		// implemented
		{
			...
			_log.info("Four Sepulchers: Beginning in Cooldown time");
		}
	}
	...
	protected void spawnManagers()
	{
		...
		for (L2Spawn spawnDat; i <= 31924; i++)
		{
			...
			try
			{
				...
				_log.info("Four Sepulchers: spawned " + spawnDat.getTemplate().getName());
			}
			catch (Exception e)
			{
	...
	private void loadMysteriousBox()
	{
			...
			PreparedStatement statement = con.prepareStatement("SELECT id, count, npc_templateid, locx, locy, locz, heading, respawn_delay, key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY id"))
		{
			...
			_log.info("Four Sepulchers: loaded " + _mysteriousBoxSpawns.size() + " Mysterious-Box spawns.");
		}
		catch (Exception e)
		{
	...
	private void loadPhysicalMonsters()
	{
			...
			PreparedStatement statement1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			_log.info("Four Sepulchers: loaded " + loaded + " Physical type monsters spawns.");
		}
		catch (Exception e)
		{
			// problem with initializing spawn, go to next one
	...
	private void loadMagicalMonsters()
	{
			...
			PreparedStatement statement1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			_log.info("Four Sepulchers: loaded " + loaded + " Magical type monsters spawns.");
		}
		catch (Exception e)
		{
			// problem with initializing spawn, go to next one
			...
			PreparedStatement statement1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			try (ResultSet rset1 = statement1.executeQuery())
			{
				try (PreparedStatement statement2 = con.prepareStatement("SELECT id, count, npc_templateid, locx, locy, locz, heading, respawn_delay, key_npc_id FROM four_sepulchers_spawnlist WHERE key_npc_id = ? AND spawntype = ? ORDER BY id"))
				{
					...
					_log.info("Four Sepulchers: loaded " + loaded + " Church of duke monsters spawns.");
				}
			}
		}
		catch (Exception e)
	...
	private void loadEmperorsGraveMonsters()
	{
			...
			PreparedStatement statement1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			_log.info("Four Sepulchers: loaded " + loaded + " Emperor's grave NPC spawns.");
		}
		catch (Exception e)
		{
			// problem with initializing spawn, go to next one
	...
	public synchronized void tryEntry(L2Npc npc, L2PcInstance player)
	{
		...
		if (Config.FS_PARTY_MEMBER_COUNT > 1)
		{
			...
			{
				QuestState qs = mem.getQuestState(Q00620_FourGoblets.class.getSimpleName());
		...
		{
			QuestState qs = player.getQuestState(Q00620_FourGoblets.class.getSimpleName());
			if ((qs == null) || (!qs.isStarted() && !qs.isCompleted()))
			{
	...
	public void deleteAllMobs()
	{
		for (L2Npc mob : _allMobs)
		{
			...
			{
				_log.log(Level.SEVERE, "Four Sepulchers: Failed deleting mob.", e);
			}
		}
		_allMobs.clear();
	}
	...
	protected void closeAllDoors()
	{
		for (int doorId : _hallGateKeepers.values())
		{
			try
			{
				...
				{
					_log.warning("Four Sepulchers: Attempted to close undefined door. doorId: " + doorId);
				}
			}
			catch (Exception e)
			{
				_log.log(Level.SEVERE, "Four Sepulchers: Failed closing door", e);
			}
		}
	}
	...
	public void managerSay(byte min)
	{
		...
		if (_inAttackTime)
		{
			...
			for (L2Spawn temp : _managers)
			{
				if (temp == null)
				{
					_log.warning("Four Sepulchers: managerSay(): manager is null");
					continue;
				}
				if (!(temp.getLastSpawn() instanceof L2SepulcherNpcInstance))
				{
					_log.warning("Four Sepulchers: managerSay(): manager is not Sepulcher instance");
					continue;
				}
		...
		else if (_inEntryTime)
		{
			...
			for (L2Spawn temp : _managers)
			{
				if (temp == null)
				{
					_log.warning("Four Sepulchers: Something goes wrong in managerSay()...");
					continue;
				}
				if (!(temp.getLastSpawn() instanceof L2SepulcherNpcInstance))
				{
	...
	protected class ChangeEntryTime implements Runnable
	{
		...
		public void run()
		{
			// _log.info("Four Sepulchers:In Entry Time");
			_inEntryTime = true;
			_inWarmUpTime = false;
			_inAttackTime = false;
			_inCoolDownTime = false;
	...
	protected class ChangeWarmUpTime implements Runnable
	{
		...
		public void run()
		{
			// _log.info("Four Sepulchers:In Warm-Up Time");
			_inEntryTime = true;
			_inWarmUpTime = false;
			_inAttackTime = false;
			_inCoolDownTime = false;
	...
	protected class ChangeAttackTime implements Runnable
	{
		...
		public void run()
		{
			// _log.info("Four Sepulchers:In Attack Time");
			_inEntryTime = false;
			_inWarmUpTime = false;
			_inAttackTime = true;
			_inCoolDownTime = false;
	...
	protected class ChangeCoolDownTime implements Runnable
	{
		...
		public void run()
		{
			// _log.info("Four Sepulchers:In Cool-Down Time");
			_inEntryTime = false;
			_inWarmUpTime = false;
			_inAttackTime = false;
			...
			_log.info("Four Sepulchers: Entry time: " + time.getTime());
			if (_firstTimeRun)
			{
