...
public class ZoneManager
{
	...
	{
		load();
		_log.info("Loaded " + _classZones.size() + " class zones and " + getSize() + " zones.");
	}
	...
	private final void load()
	{
		...
		try
		{
			...
			List<File> hash = new ArrayList<>(files.length);
			for (File f : files)
			{
				// default file first
			...
			NamedNodeMap attrs;
			Node attribute;
			String zoneName, newZoneType;
			int[][] coords;
			int zoneId, minZ, maxZ;
			String zoneType = null, zoneShape;
					...
					if ("list".equalsIgnoreCase(n.getNodeName()))
					{
						...
						for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
						{
							if ("zone".equalsIgnoreCase(d.getNodeName()))
							{
								...
								if (attribute != null)
								{
									newZoneType = attribute.getNodeValue();
								}
								else
								{
