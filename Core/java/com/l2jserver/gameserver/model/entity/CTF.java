...
public class CTF
{
			...
			for (L2PcInstance player : _players)
			{
				if (player != null)
				{
					...
					if (player.isMageClass())
					{
						...
						{
							for (int i1 : Config.CTF_EVENT_MAGE_BUFFS.keySet())
							{
					...
					else
					{
						...
						{
							for (int i2 : Config.CTF_EVENT_FIGHTER_BUFFS.keySet())
							{
	...
	public static void loadData()
	{
			...
			PreparedStatement statement = con.prepareStatement("Select * from ctf"))
		{
			...
			{
				try (PreparedStatement statement2 = con.prepareStatement("Select * from ctf_teams where teamId = ?"))
				{
	...
	public static void saveData()
	{
			...
			PreparedStatement statement = con.prepareStatement("Delete from ctf"))
		{
			statement.execute();
			try (
				PreparedStatement statement0 = con.prepareStatement("INSERT INTO ctf (eventName, eventDesc, joiningLocation, minlvl, maxlvl, npcId, npcX, npcY, npcZ, npcHeading, rewardId, rewardAmount, teamsCount, joinTime, eventTime, minPlayers, maxPlayers) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"))
			{
				statement0.setString(1, _eventName);
				statement0.setString(2, _eventDesc);
				statement0.setString(3, _joiningLocationName);
				statement0.setInt(4, _minlvl);
				statement0.setInt(5, _maxlvl);
				statement0.setInt(6, _npcId);
				statement0.setInt(7, _npcX);
				statement0.setInt(8, _npcY);
				statement0.setInt(9, _npcZ);
				statement0.setInt(10, _npcHeading);
				statement0.setInt(11, _rewardId);
				statement0.setInt(12, _rewardAmount);
				statement0.setInt(13, _teams.size());
				statement0.setInt(14, _joinTime);
				statement0.setInt(15, _eventTime);
				statement0.setInt(16, _minPlayers);
				statement0.setInt(17, _maxPlayers);
				statement0.execute();
			}
			try (PreparedStatement statement2 = con.prepareStatement("Delete from ctf_teams"))
			{
				statement2.execute();
				...
				for (String teamName : _teams)
				{
					...
					PreparedStatement statement3 = con.prepareStatement("INSERT INTO ctf_teams (teamId ,teamName, teamX, teamY, teamZ, teamColor, flagX, flagY, flagZ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
					statement3.setInt(1, index);
					statement3.setString(2, teamName);
					statement3.setInt(3, _teamsX.get(index));
					statement3.setInt(4, _teamsY.get(index));
					statement3.setInt(5, _teamsZ.get(index));
					statement3.setInt(6, _teamColors.get(index));
					statement3.setInt(7, _flagsX.get(index));
					statement3.setInt(8, _flagsY.get(index));
					statement3.setInt(9, _flagsZ.get(index));
					statement3.execute();
				}
			}
		}
