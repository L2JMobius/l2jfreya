...
public class L2DoormenInstance extends L2NpcInstance
{
	...
	public void showChatWindow(L2PcInstance player)
	{
		...
		if (!isOwnerClan(player))
		{
			html.setFile(player, "data/html/doormen/" + getTemplate().npcId + "-no.htm");
		}
		else
		{
			html.setFile(player, "data/html/doormen/" + getTemplate().npcId + ".htm");
		}
	...
	protected void cannotManageDoors(L2PcInstance player)
	{
		...
		NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile(player, "data/html/doormen/" + getTemplate().npcId + "-busy.htm");
		player.sendPacket(html);
	}
