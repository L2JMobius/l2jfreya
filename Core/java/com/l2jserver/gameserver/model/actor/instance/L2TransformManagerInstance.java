...
import com.l2jserver.scripts.quests.Q00136_MoreThanMeetsTheEye;
...
public final class L2TransformManagerInstance extends L2MerchantInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		if (command.startsWith("LearnTransformationSkill"))
		{
			...
			else
			{
				...
				html.setFile(player, htmlFolder + "master_transformation003.htm");
		...
		else if (command.startsWith("BuyTransformationItems"))
		{
			...
			else
			{
				...
				html.setFile(player, htmlFolder + "master_transformation004.htm");
				player.sendPacket(html);
			}
			return;
		}
		else if (command.startsWith("LearnSubClassSkill"))
		{
			...
			{
				NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player, htmlFolder + "master_transformation008.htm");
				player.sendPacket(html);
			}
			else
			{
		...
		else if (command.startsWith("CancelCertification"))
		{
			...
			{
				html.setFile(player, htmlFolder + "master_transformation007.htm");
			}
			...
			{
				html.setFile(player, htmlFolder + "master_transformation008no.htm");
			}
			else
			{
				...
				{
					html.setFile(player, htmlFolder + "master_transformation010no.htm");
				}
				else
				{
					for (String varName : _questVarNames)
					...
					html.setFile(player, htmlFolder + "master_transformation009no.htm");
	...
	public static boolean canTransform(L2PcInstance player)
	{
		...
		final QuestState st = player.getQuestState(Q00136_MoreThanMeetsTheEye.class.getSimpleName());
