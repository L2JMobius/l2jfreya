		...
		if (actualCommand.equalsIgnoreCase("PK_Count"))
		{
			...
			if ((player.getFame() >= 5000) && (player.getClassId().level() >= 2) && (player.getClan() != null) && (player.getClan().getLevel() >= 5))
			{
				if (player.getPkKills() > 0)
				{
					...
					html.setFile(player, "data/html/famemanager/" + getNpcId() + "-3.htm");
				}
				else
				{
					html.setFile(player, "data/html/famemanager/" + getNpcId() + "-4.htm");
				}
			}
			else
			{
				html.setFile(player, "data/html/famemanager/" + getNpcId() + "-lowfame.htm");
			}
		...
		else if (actualCommand.equalsIgnoreCase("CRP"))
		{
			...
			if ((player.getClan() != null) && (player.getClan().getLevel() >= 5))
			{
				if ((player.getFame() >= 1000) && (player.getClassId().level() >= 2))
				{
					...
					html.setFile(player, "data/html/famemanager/" + getNpcId() + "-5.htm");
				}
				else
				{
					html.setFile(player, "data/html/famemanager/" + getNpcId() + "-lowfame.htm");
			...
			{
				html.setFile(player, "data/html/famemanager/" + getNpcId() + "-noclan.htm");
			}
