...
public class L2AchievementsInstance extends L2NpcInstance
{
		...
		else if (command.startsWith("getReward"))
		{
			...
			else if ((id == 20))
			{
				...
				if (player.isClanLeader())
				{
					...
					{
						player.sendMessage("Your clan level must be 8.");
					}
					
				}
				else
				{
					player.sendMessage("This is only for clan leaders.");
				}
				
			}
			else if ((id == 10))
			{
				if ((player.isHero()) && (!AchievementsManager.getInstance().isBinded(player.getCharId(), id)))
				{
					AchievementsManager.getInstance().getBinded().add(player.getCharId() + "@" + id);
					player.saveAchievementData(id, player.getCharId());
					AchievementsManager.getInstance().rewardForAchievement(id, player);
				}
				else
				{
					player.sendMessage("Hero was already rewarded for this achievement.");
				}
			}
			else if ((id == 11))
			{
				if ((player.isNoble()) && (!AchievementsManager.getInstance().isBinded(player.getCharId(), id)))
				{
					AchievementsManager.getInstance().getBinded().add(player.getCharId() + "@" + id);
					player.saveAchievementData(id, player.getCharId());
					AchievementsManager.getInstance().rewardForAchievement(id, player);
				...
				{
					player.sendMessage("Your Nobless Status was already rewarded for this achievement.");
				}
			}
			else
			{
	...
	public void showChatWindow(L2PcInstance player, int val)
	{
		...
		tb.append("<html><title>L2jPDT Achievements Manager</title><body><br>");
		tb.append("<center>Hello <font color=\"LEVEL\">" + player.getName() + "</font><br>This is the challenge list, check your achievements and get reward.</center>");
		tb.append("<br><img src=\"l2ui.squaregray\" width=\"270\" height=\"3\"><br>");
		tb.append("<button value=\"My Achievems\" action=\"bypass -h npc_%objectId%_showMyAchievements\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" width=270 height=25>");
		tb.append("<button value=\"Completed Achievements?\" action=\"bypass -h npc_%objectId%_showMyStats\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" width=270 height=25>");
		tb.append("<button value=\"Do you have any question ?\" action=\"bypass -h npc_%objectId%_showHelpWindow\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" width=270 height=25>");
	...
	private void showAchievementInfo(int achievementID, L2PcInstance player)
	{
		...
		{
			tb.append("<button value=\"Receive Reward!\" action=\"bypass -h npc_%objectId%_getReward " + a.getID() + "\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" width=120 height=25>");
		}
		...
		tb.append("<br><img src=\"l2ui.squaregray\" width=\"270\" height=\"1s\"><br>");
		tb.append("<center><button value=\"Back\" action=\"bypass -h npc_%objectId%_showMyAchievements\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" width=270 height=25></center>");
	...
	{
		StringBuilder tb = new StringBuilder();
		tb.append("<html><title>L2jPDT Achievements Manager</title><body><center><br>");
		...
		tb.append("<br><img src=\"l2ui.squaregray\" width=\"270\" height=\"1s\"><br>");
		tb.append("<center><button value=\"Back\" action=\"bypass -h npc_%objectId%_showMainWindow\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" width=270 height=25></center>");
