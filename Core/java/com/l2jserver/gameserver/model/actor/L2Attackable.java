...
public class L2Attackable extends L2Npc
{
	...
	// debug
	private final boolean _debug_drop_chance = Config.DEBUG_DROP_CHANCE;
	...
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		if (lastAttacker.isPremiumAccount())
		{
			// Applies Drop rates
			if (!Config.PREMIUM_RATE_DROP_ITEMS_ID.isEmpty())
			{
				if (Config.PREMIUM_RATE_DROP_ITEMS_ID.containsKey(drop.getItemId()))
				{
					dropChance *= Config.PREMIUM_RATE_DROP_ITEMS_ID.get(drop.getItemId());
					showChance(lastAttacker, dropChance, drop, true);
				}
			}
			else if (isSweep)
			{
			...
			for (L2DropCategory cat : npcTemplate.getDropData())
			{
				...
				else
				{
					...
					if (item != null)
					{
						...
						else if (isRaid() && Config.AUTO_LOOT_RAIDS)
						{
							player.doAutoLoot(this, item);
						}
						else if (vars.getBoolean("@AUTO_LOOT"))
						{
							player.doAutoLoot(this, item);
						}
						else
	 ...
	 * @param isPremium
	 */
	private void showChance(L2PcInstance lastAttacker, float dropChance, L2DropData drop, boolean isPremium)
	{
		if (_debug_drop_chance)
		{
			if (lastAttacker.isGM())
			{
				if (isPremium)
				{
					lastAttacker.sendMessage("Premium: Chance: " + dropChance + " for Item ID: " + drop.getItemId());
				}
				else
				{
					lastAttacker.sendMessage("Normal: Chance: " + dropChance + " for Item ID: " + drop.getItemId());
				}
			}
		}
	}
