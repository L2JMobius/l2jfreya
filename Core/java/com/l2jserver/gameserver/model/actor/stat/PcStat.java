...
public class PcStat extends PlayableStat
{
	...
	public boolean addExpAndSp(long addToExp, int addToSp, boolean useBonuses)
	{
		if (useBonuses)
		{
			...
			if (Config.ENABLE_VITALITY)
			{
				...
				{
					case 1:
						bonusExp += Config.RATE_VITALITY_LEVEL_1;
						bonusSp += Config.RATE_VITALITY_LEVEL_1;
						break;
					case 2:
						bonusExp += Config.RATE_VITALITY_LEVEL_2;
						bonusSp += Config.RATE_VITALITY_LEVEL_2;
						break;
					case 3:
						bonusExp += Config.RATE_VITALITY_LEVEL_3;
						bonusSp += Config.RATE_VITALITY_LEVEL_3;
						break;
					case 4:
						bonusExp += Config.RATE_VITALITY_LEVEL_4;
						bonusSp += Config.RATE_VITALITY_LEVEL_4;
						break;
				}
			}
			
			// apply rune bonus
			double[] bonus = BonusExpManager.getInstance().getBonusExpAndSp(getActiveChar());
			bonusExp += bonus[0];
			bonusSp += bonus[1];
			
			// apply recomendation bonus
			bonusExp += RecoBonus.getRecoMultiplier(getActiveChar());
			bonusSp += RecoBonus.getRecoMultiplier(getActiveChar());
			
			// calculate exp after bonus
			addToExp *= bonusExp;
			addToSp *= bonusSp;
			
			if (Config.DEBUG)
			{
				_log.info("Bonus Exp: " + ((bonusExp - 1) * 100) + "% Bonus SP: " + ((bonusSp - 1) * 100) + "%");
			}
	...
	public final boolean addLevel(byte value)
	{
		...
		if (levelIncreased)
		{
			...
			{
				QuestState qs = getActiveChar().getQuestState(Q00255_Tutorial.class.getSimpleName());
				if (qs != null)
				{
