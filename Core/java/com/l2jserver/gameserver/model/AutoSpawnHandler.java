...
public class AutoSpawnHandler
{
	...
	private AutoSpawnHandler()
	{
		...
		restoreSpawnData();
		_log.info(getText("TG9hZGVkIFJlZy4gQXV0byBTcGF3bnM6IA==") + _registeredSpawns.size());
		_log.info(getText("TG9hZGVkIFJlZy4gUnVubmluZyBTcGF3bnM6IA==") + _runningSpawns.size());
	}
	...
	private void restoreSpawnData()
	{
			...
			PreparedStatement ps = con.prepareStatement("SELECT * FROM random_spawn_loc WHERE groupId=?"))
		{
			...
			_log.info("Loaded " + numLoaded + " Auto Spawns.");
		}
		catch (Exception e)
		{
	...
	public static class AutoSpawnInstance
	{
		...
		/*
		 * public L2Npc[] getNPCInstanceList() { L2Npc[] ret; synchronized (_npcList) { ret = new L2Npc[_npcList.size()]; _npcList.toArray(ret); } return ret; } public L2Spawn[] getSpawns() { List<L2Spawn> npcSpawns = new ArrayList<>(); for (L2Npc npcInst : _npcList) {
		 * npcSpawns.add(npcInst.getSpawn()); } return npcSpawns.toArray(new L2Spawn[npcSpawns.size()]); }
		 */
