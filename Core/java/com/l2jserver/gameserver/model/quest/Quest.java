...
public class Quest
{
	...
	// Variables for multiclass npc - for future usage is better to be there
	public static final String CLASS_COUNT_1 = "CLASS_COUNT_1";
	public static final String CLASS_COUNT_2 = "CLASS_COUNT_2";
	public static final String CLASS_COUNT_3 = "CLASS_COUNT_3";
	...
	public Quest(int questId, String name, String descr)
	{
		...
		_name = name;
		_descr = descr;
		_path = getClass().getCanonicalName();
	...
	public final static void playerEnter(L2PcInstance player)
	{
			...
			PreparedStatement statement = con.prepareStatement("SELECT name,value FROM character_quests WHERE charId=? AND var=?"))
		{
			...
			try (PreparedStatement statement2 = con.prepareStatement("SELECT name,var,value FROM character_quests WHERE charId=? AND var<>?"))
			{
				statement2.setInt(1, player.getObjectId());
				statement2.setString(2, "<state>");
	...
	{
		String content = HtmCache.getInstance().getHtm(player, "data/scripts/" + getDescr().toLowerCase() + "/" + getName() + "/" + fileName);
		
		if (content == null)
		{
			content = HtmCache.getInstance().getHtm(player, "data/scripts/quests/Q" + getName() + "/" + fileName);
			if (content == null)
			{
				content = HtmCache.getInstance().getHtmForce(player, "data/scripts/quests/" + getName() + "/" + fileName);
			}
		}
	...
	{
		return Config.QUEST_RESET_HOUR;
	}
	...
	{
		return Config.QUEST_RESET_MINUTES;
	}
	...
	public void SkillSeeNpcs(Collection<Integer> npcIds)
	{
		EventId(QuestEventType.ON_SKILL_SEE, npcIds);
	}
	...
	{
		return NpcEventId(npcId, QuestEventType.ON_SKILL_LEARN);
	}
	
	public L2NpcTemplate addTrapActionId(int trapId)
	{
		return NpcEventId(trapId, QuestEventType.ON_TRAP_ACTION);
	}
	...
	public void EnterZoneId(int... zoneIds)
	{
		EventId(QuestEventType.ON_ENTER_ZONE, zoneIds);
	}
	...
	public void ExitZoneId(int... zoneIds)
	{
		EventId(QuestEventType.ON_ENTER_ZONE, zoneIds);
	}
	...
	public static final String startClassLv = Config.MULTICLASS_START_LV;
	
	/**
	 * @param player
	 */
	public static void checkTheClassCount(L2PcInstance player)
	{
		final PlayerVariables vars = player.getVariables();
		if (vars.getInt(CLASS_COUNT_1, 0) == 0)
		{
			vars.set(CLASS_COUNT_1, 1);
		}
		else if (vars.getInt(CLASS_COUNT_1, 0) != 0)
		{
			if (vars.getInt(CLASS_COUNT_2, 0) != 0)
			{
				vars.set(CLASS_COUNT_3, 1);
			}
			else
			{
				vars.set(CLASS_COUNT_2, 1);
			}
		}
	}
	 ...
	 * @param player
	 */
	public static void addReward(L2PcInstance player)
	{
		player.addItem("Multiclass", Config.MULTICLASS_REW_ID_1, Config.MULTICLASS_REW_COUNT_1, null, true);
		player.addItem("Multiclass", Config.MULTICLASS_REW_ID_2, Config.MULTICLASS_REW_COUNT_2, null, true);
		player.addItem("Multiclass", Config.MULTICLASS_REW_ID_3, Config.MULTICLASS_REW_COUNT_3, null, true);
	}
	
	/**
	 * @param player
	 */
	public static void changelevel(L2PcInstance player)
	{
		byte lvl = Byte.parseByte(startClassLv);
	 ...
	 * @param player
	 */
	public static void statusUpdate(L2PcInstance player)
	{
		player.broadcastUserInfo();
		player.broadcastInfo();
		player.broadcastStatusUpdate();
		player.sendPacket(new ExShowScreenMessage("Congratulation, your class changed.", 5000));
	}
	
	/**
	 * @param player
	 */
	public static void classDone(L2PcInstance player)
	{
		player.sendMessage("You cannot do again this class transfer. Already done.");
		player.sendPacket(new ExShowScreenMessage("You cannot do again this class transfer. Already done.", 5000));
	 ...
	 * @param player
	 */
	public static void maxiumClasses(L2PcInstance player)
	{
		player.sendMessage("You cannot change now. You reached maximum allowed classes.");
		player.sendPacket(new ExShowScreenMessage("You cannot change now. You reached maximum allowed classes.", 5000));
		player.sendPacket(new Earthquake(player.getX(), player.getY(), player.getZ(), 25, 6));
		player.sendPacket(new ExRedSky(6));
		player.sendPacket(new SocialAction(player, 5));
	}
	 ...
	 * @param player
	 */
	public static void noKamael(L2PcInstance player)
	{
		player.sendMessage("Kamael change class only with Kamaels.");
		player.sendPacket(new ExShowScreenMessage("Kamael change class only with Kamaels.", 5000));
		player.sendPacket(new Earthquake(player.getX(), player.getY(), player.getZ(), 25, 6));
		player.sendPacket(new ExRedSky(6));
		player.sendPacket(new SocialAction(player, 5));
	}
