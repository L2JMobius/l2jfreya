...
public final class ItemAuctionInstance
{
	...
	private final ItemAuction loadAuction(final int auctionId) throws SQLException
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();)
		{
			...
			try (PreparedStatement statement = con.prepareStatement("SELECT auctionItemId,startingTime,endingTime,auctionStateId FROM item_auction WHERE auctionId=?"))
			{
			...
			try (PreparedStatement statement = con.prepareStatement("SELECT playerObjId,playerBid FROM item_auction_bid WHERE auctionId=?"))
			{
				statement.setInt(1, auctionId);
				try (ResultSet rset = statement.executeQuery())
				{
