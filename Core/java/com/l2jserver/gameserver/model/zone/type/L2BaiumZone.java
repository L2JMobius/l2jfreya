 ...
 * @author L2jPDT
 */
public class L2BaiumZone extends L2ZoneType
{
	public L2BaiumZone(final int id)
	{
		super(id);
	}
	...
	protected void onEnter(final L2Character character)
	{
		if (character instanceof L2PcInstance)
		{
			...
			if ((character.getLevel() >= Config.BAIUM_ZONE_MAX_LV) && (!character.isGM()))
			{
				character.teleToLocation(47591, 185820, -3486);
				character.sendPacket(new ExShowScreenMessage("You cannot to enter, if you have " + Config.BAIUM_ZONE_MAX_LV + " level or higher", 5000));
				character.stopSkillEffects(1323);
			}
			if (Config.BAIUM_ZONE_GIVE_NOBLESS)
			{
		...
		{
			character.setInsideZone(L2Character.ZONE_ALTERED, true);
			if (character.getLevel() >= Config.BAIUM_ZONE_MAX_LV)
			{
				character.teleToLocation(47591, 185820, -3486);
				character.sendPacket(new ExShowScreenMessage("You cannot to enter, if you have " + Config.BAIUM_ZONE_MAX_LV + " level or higher", 5000));
				character.stopSkillEffects(1323);
