...
public final class L2ItemInstance extends L2Object
{
	...
	public final void pickupMe(L2Character player)
	{
		...
		if (!Config.DISABLE_TUTORIAL && ((itemId == 57) || (itemId == 6353)))
		{
			...
			if (actor != null)
			{
				QuestState qs = actor.getQuestState(Q00255_Tutorial.class.getSimpleName());
				if (qs != null)
				{
	...
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement1 = con.prepareStatement("SELECT augAttributes,augSkillId,augSkillLevel FROM item_attributes WHERE itemId=?");
			PreparedStatement statement2 = con.prepareStatement("SELECT elemType,elemValue FROM item_elementals WHERE itemId=?");)
		{
	...
	{
		try (Connection con = pooledCon == null ? L2DatabaseFactory.getInstance().getConnection() : pooledCon;
			PreparedStatement statement1 = con.prepareStatement("DELETE FROM item_elementals WHERE itemId = ?");
			PreparedStatement statement2 = con.prepareStatement("INSERT INTO item_elementals VALUES(?,?,?)"))
		{
			statement1.setInt(1, getObjectId());
			...
			{
				statement2.setInt(1, getObjectId());
				statement2.setByte(2, elm.getElement());
				statement2.setInt(3, elm.getValue());
