...
import com.l2jserver.scripts.quests.Q00697_DefendtheHallofErosion;
import com.l2jserver.util.Rnd;

public class HeartInfinityDefence extends SeedOfInfinityController
{
	...
	{
		super(-1, HeartInfinityDefence.class.getSimpleName(), "Heart Infinity Defence");
		StartNpcs(SeedOfInfinityController.GATEKEEPER_OF_ABYSS_YELLOW, SeedOfInfinityController.DESTROYED_TUMOR_2, SeedOfInfinityController.DESTROYED_TUMOR_3_START);
		TalkNpcs(SeedOfInfinityController.GATEKEEPER_OF_ABYSS_YELLOW, SeedOfInfinityController.DESTROYED_TUMOR_2, SeedOfInfinityController.DESTROYED_TUMOR_3_START);
		SpawnNpcs(SeedOfInfinityController.HID_NOT_MOVE);
		SpawnNpcs(SeedOfInfinityController.SOUL_DEVOURER_3);
		AggroRangeEnterNpcs(SeedOfInfinityController.WARD_OF_DEATH_2);
		KillNpcs(SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81, SeedOfInfinityController.SOUL_COFIN_2);
		EnterZoneId(SeedOfInfinityController.EKIMUS_ZONE);
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		{
			if ((partyMember.getLevel() < Config.HEART_INFINITY_DEFENCE_MIN_LV) || (partyMember.getLevel() > Config.HEART_INFINITY_DEFENCE_MAX_LV))
			{
			...
			Long reentertime = InstanceManager.getInstance().getInstanceTime(partyMember.getObjectId(), SeedOfInfinityController.INSTANCE_ID_HEART_INFINITY_DEFENCE);
	...
	protected void enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		if (checkConditions(player))
		{
			...
			world.setTemplateId(SeedOfInfinityController.INSTANCE_ID_HEART_INFINITY_DEFENCE);
			world.setStatus(0);
	...
	private void conquestBegins(final HIDWorld world)
	{
		ThreadPoolManager.getInstance().scheduleGeneral(() ->
		{
			...
			for (int[] spawn1 : SeedOfInfinityController.ROOMS_MOBS_HID)
			{
			...
			for (int[] spawn2 : SeedOfInfinityController.ROOMS_TUMORS_HID)
			{
			...
			preEkimus = addSpawn(SeedOfInfinityController.PRE_EKIMUS, -179534, 208510, -15496, 16342, false, 0, false, world.getInstanceId());
			...
			ThreadPoolManager.getInstance().scheduleGeneral(() ->
			{
				if (!conquestEnded)
				{
					...
					for (int[] spawn : SeedOfInfinityController.ROOMS_ALIVE_TUMORS_HID)
					{
			...
			world.wagonSpawnTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(() -> addSpawn(SeedOfInfinityController.SOUL_DEVOURER_3, -179544, 207400, -15496, 0, false, 0, false, world.getInstanceId()), 1000, wagonRespawnTime);
		}, 20000);
	}
	
	void spawnCoffin(L2Npc npc, HIDWorld world)
	{
		spawnNpc(SeedOfInfinityController.SOUL_COFIN, npc.getLocationXYZ(), 0, world.getInstanceId());
	}
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (npcId == SeedOfInfinityController.GATEKEEPER_OF_ABYSS_YELLOW)
		{
			enterInstance(player, SeedOfInfinityController.HEART_INFINITY_DEFENCE_XML, SeedOfInfinityController.ENTER_TELEPORT_HID);
		}
		return "";
	}
	...
	{
		if (Util.contains(SeedOfInfinityController.HID_NOT_MOVE, npc.getNpcId()))
		{
		...
		if (tmpworld instanceof HIDWorld)
		{
			if (npc.getNpcId() == SeedOfInfinityController.SOUL_DEVOURER_3)
			{
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (tmpworld instanceof HIDWorld)
		{
			...
			switch (npc.getNpcId())
			{
				case SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81:
					...
					ThreadPoolManager.getInstance().scheduleGeneral(() ->
					{
						...
						L2Npc alivetumor = spawnNpc(SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81, loc, 0, world.getInstanceId());
					...
					break;
				case SeedOfInfinityController.SOUL_COFIN_2:
	...
	private class TimerTask implements Runnable
	{
		...
		public void run()
		{
			...
			else
			{
				if (time == 15)
				{
					for (int[] spawn : SeedOfInfinityController.ROOMS_BOSSES_HID)
					{
						for (int i = 0; i < spawn[6]; i++)
						{
