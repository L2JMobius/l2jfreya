...
public class HallOfErosionDefence extends SeedOfInfinityController
{
	...
	{
		super(-1, HallOfErosionDefence.class.getSimpleName(), "Hall Of Erosion Defence");
		StartNpcs(SeedOfInfinityController.EKIMUS_MOUTH, SeedOfInfinityController.DESTROYED_TUMOR_2);
		TalkNpcs(SeedOfInfinityController.EKIMUS_MOUTH, SeedOfInfinityController.DESTROYED_TUMOR_2);
		SpawnNpcs(SeedOfInfinityController.HOED_NOT_MOVE);
		SpawnNpcs(SeedOfInfinityController.SEED);
		AggroRangeEnterNpcs(SeedOfInfinityController.WARD_OF_DEATH_2);
		KillNpcs(SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81, SeedOfInfinityController.SEED, SeedOfInfinityController.SOUL_COFIN_2);
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		{
			if ((partyMember.getLevel() < Config.EROSION_DEFENCE_MIN_LEVEL_75) || (partyMember.getLevel() > Config.EROSION_DEFENCE_MAX_LEVEL_85))
			{
			...
			Long reentertime = InstanceManager.getInstance().getInstanceTime(partyMember.getObjectId(), SeedOfInfinityController.INSTANCE_ID_HALL_OF_EROSION_DEFENCE);
	...
	protected void enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		if (checkConditions(player))
		{
			...
			world.setTemplateId(SeedOfInfinityController.INSTANCE_ID_HALL_OF_EROSION_DEFENCE);
			world.setStatus(0);
			...
			if (player.isInParty())
			{
				for (L2PcInstance partyMember : player.getParty().isInCommandChannel() ? player.getParty().getCommandChannel().getMembers() : player.getParty().getPartyMembers())
				{
	...
	{
		for (int[] spawn : SeedOfInfinityController.ROOMS_MOBS_HOED)
		{
		...
		for (int[] spawn : SeedOfInfinityController.TUMOR_DEAD_SPAWN_HOED)
		{
			for (int i = 0; i < spawn[6]; i++)
			{
		...
		for (int[] spawn : SeedOfInfinityController.SEEDS_SPAWN_HOED)
		{
			for (int i = 0; i < spawn[6]; i++)
			{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(() ->
		{
			if (!conquestEnded)
			{
				...
				for (int[] spawn : SeedOfInfinityController.TUMOR_ALIVE_SPAWN_HOED)
				{
					for (int i = 0; i < spawn[6]; i++)
					{
						L2Npc npc = addSpawn(spawn[0], spawn[1], spawn[2], spawn[3], spawn[4], false, 0, false, world.getInstanceId());
	...
	{
		if (npc.getNpcId() == SeedOfInfinityController.EKIMUS_MOUTH)
		{
			enterInstance(player, SeedOfInfinityController.HALL_OF_EROSION_DEFENCE_XML, SeedOfInfinityController.ENTER_TELEPORT_HOED);
			return "";
		}
		return "";
	}
		...
		if (tmpworld instanceof HEDWorld)
		{
			...
			switch (npc.getNpcId())
			{
				case SeedOfInfinityController.WARD_OF_DEATH_2:
					...
					{
						spawnNpc(SeedOfInfinityController.GROUP_SPAWN_LIST_HOEA[Rnd.get(SeedOfInfinityController.GROUP_SPAWN_LIST_HOEA.length)], npc.getLocationXYZ(), 0, world.getInstanceId());
					}
					npc.doDie(npc);
					break;
				case SeedOfInfinityController.SYMBOL_OF_COHEMENES:
					for (int i = 0; i < Rnd.get(1, 4); i++)
					{
						spawnNpc(SeedOfInfinityController.GROUP_SPAWN_LIST_HOEA[Rnd.get(SeedOfInfinityController.GROUP_SPAWN_LIST_HOEA.length)], npc.getLocationXYZ(), 0, world.getInstanceId());
					}
	...
	{
		if (Util.contains(SeedOfInfinityController.HOED_NOT_MOVE, npc.getNpcId()))
		{
			npc.setIsNoRndWalk(true);
			npc.setIsImmobilized(true);
		}
		
		if (npc.getNpcId() == SeedOfInfinityController.SEED)
		{
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (tmpworld instanceof HEDWorld)
		{
			...
			if (npc.getNpcId() == SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81)
			{
				...
				world.deadTumor = spawnNpc(SeedOfInfinityController.DESTROYED_TUMOR_2, npc.getLocationXYZ(), 0, world.getInstanceId());
				...
				ThreadPoolManager.getInstance().scheduleGeneral(() ->
				{
					...
					L2Npc tumor = spawnNpc(SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81, world.deadTumor.getLocationXYZ(), 0, world.getInstanceId());
	...
	private class RegenerationCoffinSpawn implements Runnable
	{
		...
		public void run()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				L2Npc worm = spawnNpc(SeedOfInfinityController.SOUL_COFIN, _npc.getLocationXYZ(), 0, _world.getInstanceId());
