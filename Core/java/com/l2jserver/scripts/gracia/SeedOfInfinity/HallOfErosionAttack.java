...
import com.l2jserver.scripts.quests.Q00696_ConquertheHallofErosion;
import com.l2jserver.util.Rnd;

public class HallOfErosionAttack extends SeedOfInfinityController
{
	...
	{
		super(-1, HallOfErosionAttack.class.getSimpleName(), "Hall Of Erosion Attack");
		...
		SpawnNpcs(SeedOfInfinityController.COHEMENES);
		SpawnNpcs(SeedOfInfinityController.HOEA_NOT_MOVE);
		AggroRangeEnterNpcs(SeedOfInfinityController.WARD_OF_DEATH_2, SeedOfInfinityController.SYMBOL_OF_COHEMENES);
		AttackNpcs(SeedOfInfinityController.COHEMENES);
		KillNpcs(SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81, SeedOfInfinityController.COHEMENES, SeedOfInfinityController.SOUL_COFIN_2);
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		{
			if ((partyMember.getLevel() < Config.EROSION_ATTACK_MIN_LEVEL_75) || (partyMember.getLevel() > Config.EROSION_ATTACK_MAX_LEVEL_85))
			{
			...
			Long reentertime = InstanceManager.getInstance().getInstanceTime(partyMember.getObjectId(), SeedOfInfinityController.INSTANCE_ID_HALL_OF_EROSION_ATTACK);
	...
	protected int enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		world = new HEAWorld();
		world.setTemplateId(SeedOfInfinityController.INSTANCE_ID_HALL_OF_EROSION_ATTACK);
	...
	{
		for (int[] spawn : SeedOfInfinityController.ROOMS_MOBS_HOEA)
		{
		...
		for (int[] spawn : SeedOfInfinityController.ROOMS_TUMORS_HOEA)
		{
			for (int i = 0; i < spawn[6]; i++)
			{
				L2Npc npc = addSpawn(spawn[0], spawn[1], spawn[2], spawn[3], spawn[4], false, 0, false, world.getInstanceId());
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		{
			enterInstance(player, SeedOfInfinityController.HALL_OF_EROSION_ATTACK_XML, SeedOfInfinityController.ENTER_TELEPORT_HOEA);
			return "";
		}
		return "";
	}
	...
	{
		if (Util.contains(SeedOfInfinityController.HOEA_NOT_MOVE, npc.getNpcId()))
		{
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (tmpworld instanceof HEAWorld)
		{
			...
			{
				world.addTumorCount(-1);
				((L2MonsterInstance) npc).dropItem(player, SeedOfInfinityController.TEARS_OF_A_FREED_SOUL, Rnd.get(2, 5));
				npc.deleteMe();
				...
				if ((world.tumorCount == 0) && (world.cohemenes == null))
				{
					...
					int[] spawn = SeedOfInfinityController.COHEMENES_SPAWN_HOEA[Rnd.get(0, SeedOfInfinityController.COHEMENES_SPAWN_HOEA.length - 1)];
			...
			if (npc.getNpcId() == SeedOfInfinityController.COHEMENES)
			{
	...
	private class TumorRevival implements Runnable
	{
		...
		public void run()
		{
			...
			L2Npc tumor = spawnNpc(SeedOfInfinityController.TUMOR_OF_DEATH_ALIVE_81, _deadTumor.getLocationXYZ(), 0, _world.getInstanceId());
