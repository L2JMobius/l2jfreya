 ...
 */
package com.l2jserver.scripts.gracia.SeedOfDestruction;
...
public class Stage1 extends Quest
{
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		else if ((channel.getMemberCount() < Config.SEED_OF_DESTRUCTION_MIN_PLAYERS) || (channel.getMemberCount() > Config.SEED_OF_DESTRUCTION_MAX_PLAYERS))
		{
		...
		{
			if (partyMember.getLevel() < Config.SEED_OF_DESTRUCTION_MIN_LEVEL)
			{
	...
	protected void setInstanceTimeRestrictions(SOD1World world)
	{
		...
		reenter.set(Calendar.MINUTE, Config.SEED_OF_DESTRUCTION_RESET_MIN);
		reenter.set(Calendar.HOUR_OF_DAY, Config.SEED_OF_DESTRUCTION_RESET_HOUR);
		// if time is >= RESET_HOUR - roll to the next day
		if (reenter.getTimeInMillis() <= System.currentTimeMillis())
		{
			reenter.add(Calendar.DAY_OF_MONTH, 1);
		}
		if (reenter.get(Calendar.DAY_OF_WEEK) <= Config.SEED_OF_DESTRUCTION_RESET_DAY_1)
		{
			while (reenter.get(Calendar.DAY_OF_WEEK) != Config.SEED_OF_DESTRUCTION_RESET_DAY_1)
			{
				reenter.add(Calendar.DAY_OF_MONTH, 1);
			}
		}
		...
		{
			while (reenter.get(Calendar.DAY_OF_WEEK) != Config.SEED_OF_DESTRUCTION_RESET_DAY_2)
			{
				reenter.add(Calendar.DAY_OF_MONTH, 1);
			}
		}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof SOD1World)
		{
			...
			if (event.equalsIgnoreCase("Spawn"))
			{
				...
				if ((world.deviceSpawnedMobCount < Config.SEED_OF_DESTRUCTION_MAX_MOBS_SPAWNED) && (target != null) && (target.getInstanceId() == npc.getInstanceId()) && !target.isDead())
				{
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		switch (npc.getNpcId())
		{
			case ALENOS:
				InstanceWorld world = InstanceManager.getInstance().getPlayerWorld(player);
				if ((SoDManager.getSoDState() == 1) || ((world != null) && (world instanceof SOD1World)))
				{
					enterInstance(player, "SeedOfDestructionStage1.xml", ENTER_TELEPORT_1);
				}
				else if (SoDManager.getSoDState() == 2)
				{
	...
	public Stage1()
	{
		...
		load();
		StartNpcs(ALENOS, TELEPORT);
		TalkNpcs(ALENOS, TELEPORT);
		AttackNpcs(OBELISK, TIAT);
		SpawnNpcs(OBELISK, POWERFUL_DEVICE, THRONE_POWERFUL_DEVICE, TIAT_GUARD);
		KillNpcs(OBELISK, POWERFUL_DEVICE, THRONE_POWERFUL_DEVICE, TIAT, SPAWN_DEVICE, TIAT_GUARD);
