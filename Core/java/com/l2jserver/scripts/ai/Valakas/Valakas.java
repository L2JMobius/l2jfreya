package com.l2jserver.scripts.ai.Valakas;
...
public final class Valakas extends L2AttackableAIScript
{
	...
	public Valakas()
	{
		...
		switch (getStatus())
		{
			...
			case WAITING:
			{
				...
				_valakas.setCurrentHpMp(curr_hp, curr_mp);
				addBoss(_valakas);
				startQuestTimer("SPAWN_VALAKAS", Config.VALAKAS_WAIT_TIME, null, null);
				break;
			}
			case IN_FIGHT:
			{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		switch (event)
		{
			case "enter":
			{
				int VALAKAS_ALLOW_MIN_HOUR = Config.VALAKAS_ALLOW_MIN_HOUR;
				int VALAKAS_ALLOW_MAX_HOUR = Config.VALAKAS_ALLOW_MAX_HOUR;
				
				String htmltext = null;
				
				LocalDateTime date = LocalDateTime.now();
				if (Config.SPECIFIC_TIME_TO_ENTER_VALAKAS)
				{
					if ((date.getHour() < VALAKAS_ALLOW_MIN_HOUR) || (date.getHour() > VALAKAS_ALLOW_MAX_HOUR))
					{
					...
					else
					{
						...
						else if (_zone.getPlayersInside().size() >= Config.VALAKAS_MAXIMUM_PLAYERS)
						{
						...
						else
						{
							if (player.isInParty())
							{
								...
								if (isPartyLeader)
								{
									if (members.size() > (Config.VALAKAS_MAXIMUM_PLAYERS - _zone.getPlayersInside().size()))
									{
										htmltext = "31385-04.html";
									}
									else
			...
			case "SPAWN_MINION":
			{
				if (_minionCount == Config.VALAKAS_MAXIMUM_MINIONS)
				{
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		if (_zone.isCharacterInZone(killer))
		{
			if (npc.getNpcId() == VALAKAS)
			{
				...
				long respawnTime = (Config.VALAKAS_INTERVAL_OF_SPAWN + getRandom(-Config.VALAKAS_RANDOM_SPAWN, Config.VALAKAS_RANDOM_SPAWN)) * 3600000;
