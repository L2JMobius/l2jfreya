 ...
 */
package com.l2jserver.scripts.ai.Antharas;
	...
	public Antharas()
	{
		...
		switch (getStatus())
		{
			...
			case WAITING:
			{
				...
				_antharas.setCurrentHpMp(curr_hp, curr_mp);
				GrandBossManager.getInstance().addBoss(_antharas);
				startQuestTimer("SPAWN_ANTHARAS", Config.ANTHARAS_WAIT_TIME, null, null);
				break;
			}
			case IN_FIGHT:
			{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		switch (event)
		{
			case "enter":
			{
				...
				if (Config.SPECIFIC_TIME_TO_ENTER_ANTHARAS)
				{
					if ((date.getHour() < Config.ANTHARAS_ALLOW_MIN_HOUR) || (date.getHour() > Config.ANTHARAS_ALLOW_MAX_HOUR))
					{
						htmltext = "notime.html";
					}
					else
					{
						if (getStatus() == DEAD)
						...
						else if (zone.getPlayersInside().size() >= Config.ANTHARAS_MAXIMUM_PLAYERS)
						{
						...
						else if (player.isInParty())
						{
							...
							final List<L2PcInstance> members = (isInCC) ? party.getCommandChannel().getMembers() : party.getPartyMembers();
							...
							else if (members.size() > (Config.ANTHARAS_MAXIMUM_PLAYERS - zone.getPlayersInside().size()))
							{
								htmltext = "13001-04.html";
							}
							else
			...
			case "SPAWN_MINION":
			{
				if ((minionMultipler > 1) && (_minionCount < (Config.ANTHARAS_MAXIMUM_MINIONS - (minionMultipler * 2))))
				{
				...
				else if (_minionCount < (Config.ANTHARAS_MAXIMUM_MINIONS - 2))
				{
					addSpawn(BEHEMOTH, npc, true);
					addSpawn(TERASQUE, npc, true);
					_minionCount += 2;
				}
				else if (_minionCount < (Config.ANTHARAS_MAXIMUM_MINIONS - 1))
				{
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		if (zone.isCharacterInZone(killer))
		{
			if (npc.getNpcId() == ANTHARAS)
			{
				...
				long respawnTime = Config.ANTHARAS_INTERVAL_OF_SPAWN + getRandom(-Config.ANTHARAS_RANDOM_SPAWN, Config.ANTHARAS_RANDOM_SPAWN);
				respawnTime *= 3600000;
