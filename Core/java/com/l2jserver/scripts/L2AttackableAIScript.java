...
public class L2AttackableAIScript extends Quest
{
	...
	{
		super(-1, L2AttackableAIScript.class.getSimpleName(), "L2AttackableAIScript");
		...
		for (int level = 1; level < 100; level++)
		{
			...
			for (L2NpcTemplate t : templates)
			{
				try
				{
					...
					{
						NpcEventId(t.npcId, Quest.QuestEventType.ON_ATTACK);
						NpcEventId(t.npcId, Quest.QuestEventType.ON_KILL);
						NpcEventId(t.npcId, Quest.QuestEventType.ON_SPAWN);
						NpcEventId(t.npcId, Quest.QuestEventType.ON_SKILL_SEE);
						NpcEventId(t.npcId, Quest.QuestEventType.ON_FACTION_CALL);
						NpcEventId(t.npcId, Quest.QuestEventType.ON_AGGRO_RANGE_ENTER);
					}
				}
	...
	protected void attackPlayer(L2Attackable npc, L2Character playable)
	{
		npc.setIsRunning(true);
		npc.addDamageHate(playable, 0, 999);
		npc.getAI().setIntention(CtrlIntention.AI_INTENTION_ATTACK, playable);
