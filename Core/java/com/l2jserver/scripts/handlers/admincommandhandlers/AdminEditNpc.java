...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_show_scripts"))
		{
			try
			{
				...
				L2NpcTemplate npc = NpcTable.getInstance().getTemplate(npcId);
				for (Quest q : npc.getEventQuests(QuestEventType.ON_KILL))
				{
					activeChar.sendMessage("Event: " + q.getName());
				}
			}
			catch (Exception e)
			{
	...
	private void editShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		L2TradeList tradeList = BuyListTable.getInstance().getBuyList(tradeListID);
	...
	{
		List<L2TradeList> tradeLists = BuyListTable.getInstance().getBuyListByNpcId(merchantID);
		if (tradeLists == null)
		{
	...
	private void updateTradeList(int itemID, long price, int tradeListID, int order)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (Config.CUSTOM_MERCHANT_TABLES)
			{
				try (PreparedStatement stmt = con.prepareStatement("UPDATE `custom_merchant_buylists` SET `price` = ? WHERE `shop_id` = ? AND `order` = ?"))
				{
			...
			if (updated == 0)
			{
				try (PreparedStatement stmt = con.prepareStatement("UPDATE `merchant_buylists` SET `price` = ? WHERE `shop_id` = ? AND `order` = ?"))
				{
					stmt.setLong(1, price);
					stmt.setInt(2, tradeListID);
					stmt.setInt(3, order);
	...
	private void deleteTradeList(int tradeListID, int order)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (Config.CUSTOM_MERCHANT_TABLES)
			{
				try (PreparedStatement stmt = con.prepareStatement("DELETE FROM `custom_merchant_buylists` WHERE `shop_id` = ? AND `order` = ?"))
				{
			...
			if (updated == 0)
			{
				try (PreparedStatement stmt = con.prepareStatement("DELETE FROM `merchant_buylists` WHERE `shop_id` = ? AND `order` = ?"))
				{
					stmt.setInt(1, tradeListID);
					stmt.setInt(2, order);
					updated = stmt.executeUpdate();
	...
	{
		NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		String content = HtmCache.getInstance().getHtm(activeChar, "data/html/admin/editnpc.htm");
	...
	private void showNpcSkillList(L2PcInstance activeChar, int npcId, int page)
	{
		...
		StringBuffer replyMSG = new StringBuffer("<html><title>" + npcData.getName() + "(" + npcData.npcId + "): " + _skillsize + " skills</title><body>");
		replyMSG.append("<table width=300 bgcolor=666666><tr>");
		...
		replyMSG.append("</tr></table><table width=\"100%\" border=0><tr><td></td><td></td></tr>");
		...
		while (skillite.hasNext())
		{
			...
			replyMSG.append("]</a></td><td width=15><button value=\"X\" action=\"bypass -h admin_del_skill_npc " + npcData.npcId + " " + skillobj + "\" width=21 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr>");
		}
	...
	private void reloadNpcSkillList(int npcId)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			// without race
			try (PreparedStatement statement = con.prepareStatement("SELECT `skillid`, `level` FROM `npcskills` WHERE `npcid`=? AND `skillid` <> 4416"))
			{
				statement.setInt(1, npcId);
			...
			if (Config.CUSTOM_NPC_SKILLS_TABLE)
			{
				try (PreparedStatement statement2 = con.prepareStatement("SELECT `skillid`, `level` FROM `npcskills` WHERE `npcid`=? AND `skillid` <> 4416"))
				{
					statement2.setInt(1, npcId);
