	...
	private void showquestmenu(L2PcInstance target, L2PcInstance actor, String[] val)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (val[0].equals("full"))
			{
				...
				try (PreparedStatement ps = con.prepareStatement("SELECT DISTINCT name FROM character_quests WHERE charId='" + ID + "' ORDER by name"))
				{
					ps.execute();
			...
			else if (val[0].equals("name"))
			{
				...
				try (PreparedStatement ps = con.prepareStatement("SELECT var,value FROM character_quests WHERE charId='" + ID + "' and name='" + val[1] + "'"))
				{
					ps.execute();
			...
			else if (val[0].equals("var"))
			{
				...
				try (PreparedStatement req = con.prepareStatement("SELECT DISTINCT name FROM character_quests WHERE charId='" + ID + "' and var='<state>' and value='" + val[1] + "'"))
				{
					req.execute();
					try (ResultSet rs = req.getResultSet())
					{
						while (rs.next())
						{
							replyMSG.append("<tr><td><a action=\"bypass -h admin_charquestmenu " + target.getName() + " " + rs.getString(1) + "\">" + rs.getString(1) + "</a></td></tr>");
			...
			else if (val[0].equals("custom"))
			{
				...
				if (exqdb)
				{
					if (exqch)
					{
						...
						replyMSG.append("<center><table width=250><tr><td>Var</td><td>Value</td><td>New Value</td><td>&nbsp;</td></tr>");
						try (PreparedStatement req = con.prepareStatement("SELECT var,value FROM character_quests WHERE charId='" + ID + "' and name='" + qname + "'"))
						{
							req.execute();
							try (ResultSet rs = req.getResultSet())
							{
	...
	private void setquestvar(L2PcInstance target, L2PcInstance actor, String[] val)
	{
		...
		if (val[1].equals("state"))
		{
			...
			else if (val[2].equals("DELETE"))
			{
				...
				target.sendPacket(new ExShowQuestMark(qs.getQuest().getQuestId()));
			}
