...
public class AdminGB implements IAdminCommandHandler
{
		...
		switch (actualCommand.toLowerCase())
		{
			...
			case "admin_grandboss":
			{
				...
				else
				{
					...
					html.setHtml(HtmCache.getInstance().getHtm(activeChar, "data/html/admin/grandbosses/grandboss.htm"));
	...
	private void manageHtml(L2PcInstance activeChar, int grandBossId)
	{
		if (Arrays.asList(BELETH, SAILREN, ANTHARAS, VALAKAS, BAIUM, QUEENANT, ORFEN, CORE).contains(grandBossId))
		{
			...
			{
				deadStatus = 3;
				if (grandBossId == BAIUM)
				{
					switch (bossStatus)
					{
						case 0:
						{
							textColor = "00FF00"; // Yellow
							text = "Waiting";
							break;
						}
						case 1:
						{
							textColor = "FFFF00"; // Orange
							text = "In Fight";
							break;
						}
						case 2:
