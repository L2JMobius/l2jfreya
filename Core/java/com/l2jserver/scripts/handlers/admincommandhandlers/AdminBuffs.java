...
public class AdminBuffs implements IAdminCommandHandler
{
	...
	public void showBuffs(L2PcInstance activeChar, L2Character target, int page)
	{
		...
		for (int i = start; i < end; i++)
		{
			...
			if (e != null)
			{
				if (e.getSkill().getId() < 1000)
				{
					StringUtil.append(html, "<tr><td height=40><img src=icon.skill0" + e.getSkill().getId() + " width=32 height=32></td><td> ", e.getSkill().getName(), "</td><td>", e.getSkill().isToggle() ? "toggle" : (e.getAbnormalTime() - e.getTime()) + "s", "</td><td><button value=\"X\" action=\"bypass -h admin_stopbuff ", Integer.toString(target.getObjectId()), " ", String.valueOf(e.getSkill().getId()), "\" width=21 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr>");
				}
				else
				{
					StringUtil.append(html, "<tr><td height=40><img src=icon.skill" + e.getSkill().getId() + " width=32 height=32></td><td> ", e.getSkill().getName(), "</td><td>", e.getSkill().isToggle() ? "toggle" : (e.getAbnormalTime() - e.getTime()) + "s", "</td><td><button value=\"X\" action=\"bypass -h admin_stopbuff ", Integer.toString(target.getObjectId()), " ", String.valueOf(e.getSkill().getId()), "\" width=21 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr>");
				}
			}
		}
