		...
		try
		{
			...
			if (command.startsWith(COMMANDS[0])) // list
			{
				...
				if (command.length() < 10)
				{
					if (isManager)
					{
						message.setFile(activeChar, Olympiad.OLYMPIAD_HTML_PATH + "olympiad_observe.htm");
						message.replace("%objectId%", String.valueOf(target.getObjectId()));
					}
					else
					{
						message.setFile(activeChar, Olympiad.OLYMPIAD_HTML_PATH + "olympiad_arena_observe.htm");
					}
				...
				if (isManager)
				{
					message.setFile(activeChar, Olympiad.OLYMPIAD_HTML_PATH + "olympiad_observe_list.htm");
				...
				{
					message.setFile(activeChar, Olympiad.OLYMPIAD_HTML_PATH + "olympiad_arena_observe_list.htm");
					for (int i = firstArena; i <= lastArena; i++)
					{
						task = OlympiadGameManager.getInstance().getOlympiadTask(i);
						if (task != null)
