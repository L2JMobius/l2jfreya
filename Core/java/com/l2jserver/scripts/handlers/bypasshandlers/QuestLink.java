...
public class QuestLink implements IBypassHandler
{
	...
	private static final boolean NEW_STYLE = true;
	...
	{
		final StringBuilder sb = StringUtil.startAppend(150, "<html><title>" + npc.getName() + "</title><body>");
		for (Quest q : quests)
		{
			if (NEW_STYLE)
			{
				StringUtil.append(sb, "<button value=\"" + q.getDescr() + "\" action=\"bypass -h npc_", String.valueOf(npc.getObjectId()), "_Quest ", q.getName(), "\" width=270 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
				...
				if (qs != null)
				{
					...
					{
						sb.append("<center><font color=ff9900>[ In Progress ]</font></center>");
					}
					else if (qs.getState() == State.COMPLETED)
					{
						sb.append("<center><font color=00e600>[ Completed ]</font></center>");
					}
				}
				sb.append("</a><br>");
			}
	...
	public static void showQuestWindow(L2PcInstance player, L2Npc npc, String questId)
	{
		...
		if (q != null)
		{
			if (((q.getQuestId() >= 1) && (q.getQuestId() < 20000)) && ((player.getWeightPenalty() >= 3) || !player.isInventoryUnder80(true)))
			{
		...
		if (qs != null)
		{
			...
			content = HtmCache.getInstance().getHtm(player, path); // TODO path for quests html
