package com.l2jserver.scripts.ChamberOfDelusion.Square;
...
public class Square extends Quest
{
	...
	private static final long ROOM_CHANGE_INTERVAL = Config.CHAMBER_OF_DELUSION_ROOM_CHANGE_INTERVAL;
	private static final int ROOM_CHANGE_RANDOM_TIME = Config.CHAMBER_OF_DELUSION_ROOM_CHANGE_RANDOM_TIME;
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		{
			if (partyMember.getLevel() < Config.CHAMBER_OF_DELUSION_MIN_LV)
			{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if ((tmpworld != null) && (tmpworld instanceof CDWorld) && (npc.getNpcId() >= ROOM_GATEKEEPER_FIRST) && (npc.getNpcId() <= ROOM_GATEKEEPER_LAST))
		{
			...
			if (event.equals("next_room"))
			{
				...
				{
					htmltext = HtmCache.getInstance().getHtm(player, "/data/scripts/ChamberOfDelusion/no_party.htm");
				}
				else if (player.getParty().getPartyLeaderOID() != player.getObjectId())
				{
					htmltext = HtmCache.getInstance().getHtm(player, "/data/scripts/ChamberOfDelusion/no_leader.htm");
				}
				...
				{
					htmltext = HtmCache.getInstance().getHtm(player, "/data/scripts/ChamberOfDelusion/no_item.htm");
				}
			}
