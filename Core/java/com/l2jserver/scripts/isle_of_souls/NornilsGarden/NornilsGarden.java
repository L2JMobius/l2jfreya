 ...
 */
package com.l2jserver.scripts.isle_of_souls.NornilsGarden;
...
import com.l2jserver.scripts.quests.Q00179_IntoTheLargeCavern;
...
public class NornilsGarden extends Quest
{
	...
	private static final int INSTANCE_LVL_MIN = Config.NORNILS_GARDEN_MIN_LV;
	private static final int INSTANCE_LVL_MAX = Config.NORNILS_GARDEN_MAX_LV;
	...
	private static final int[] ENTER = { -111184, 74540, -12430 };
	private static final int[] EXIT = { -74058, 52040, -3680 };
	...
	private static final L2Skill WIND_WALK_FOR_BEGINNERS = SkillTable.getInstance().getInfo(4322, 1);
	private static final L2Skill HASTE_FOR_BEGINNERS = SkillTable.getInstance().getInfo(4327, 1);
	private static final L2Skill ACUMEN_FOR_BEGINNERS = SkillTable.getInstance().getInfo(4329, 1);
	private static final L2Skill BODY_FOR_BEGINNERS = SkillTable.getInstance().getInfo(4324, 1);
	
	// ITEMS
	private static final int HERB = 18478;
	...
	{
		if (WIND_WALK_FOR_BEGINNERS != null)
		{
			WIND_WALK_FOR_BEGINNERS.getEffects(ch, ch);
		}
		if (HASTE_FOR_BEGINNERS != null)
		{
			HASTE_FOR_BEGINNERS.getEffects(ch, ch);
		}
		if (ACUMEN_FOR_BEGINNERS != null)
		{
			ACUMEN_FOR_BEGINNERS.getEffects(ch, ch);
		}
		if (BODY_FOR_BEGINNERS != null)
		{
			BODY_FOR_BEGINNERS.getEffects(ch, ch);
		}
	}
	...
	private void exitInstance(L2PcInstance player)
	{
		...
		if (inst instanceof NornilsWorld)
		{
			...
			player.teleToLocation(EXIT[0], EXIT[1], EXIT[2], true);
		}
	}
	...
	private final synchronized String enterInstance(L2Npc npc, L2PcInstance player)
	{
		...
		if (world != null)
		{
			...
			if (inst != null)
			{
				teleportPlayer(player, ENTER, world.instanceId);
			}
			return null;
		}
		...
		else
		{
			...
			if (party != null)
			{
				for (L2PcInstance partyMember : party.getPartyMembers())
				{
					...
					teleportPlayer(partyMember, ENTER, instanceId);
				}
			}
			return null;
		}
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (Util.contains(FINAL_GATES, npc.getNpcId()))
		{
	...
	{
		if (Util.contains(FINAL_GATES, npc.getNpcId()))
		{
	...
	{
		if ((npc.getNpcId() == HERB) && !npc.isDead())
		{
	...
	public NornilsGarden()
	{
		...
		StartNpcs(GARDEN_GUARD);
		FirstTalkNpcs(GARDEN_GUARD);
		TalkNpcs(GARDEN_GUARD);
		KillNpcs(GATEKEEPERS[0]);
		EnterZoneId(AUTO_GATES[0]);
		TalkNpcs(FINAL_GATES);
		AttackNpcs(HERB);
