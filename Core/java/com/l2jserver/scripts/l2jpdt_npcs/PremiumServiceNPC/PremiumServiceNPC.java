 ...
 */
package com.l2jserver.scripts.l2jpdt_npcs.PremiumServiceNPC;
 ...
 * @author L2jPDT
 */
public class PremiumServiceNPC extends Quest
{
	// NPCS
	...
	public PremiumServiceNPC()
	{
		...
		{
			_log.info("L2jPDT: Premium Service is enabled, NPC loaded.");
		}
		else
		{
			_log.info("L2jPDT: Premium Service is disabled, NPC will not work.");
		}
	}
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (Config.USE_PREMIUMSERVICE)
		{
			htmltext = "<html><body><title>Premium Service - Nicol</title><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br>Hello, dear <font color=LEVEL>%player%</font><br>We must protect our world, so <font color=LEVEL>lets be a premium.</font><br>Premium account cost <font color=0099FF>Blue Eva</font> you can buy it, also can dropped from monsters.<br1>You are now level <font color=LEVEL>%level%</font>, premium accounts can be applied from level <font color=LEVEL>" + MIN_LEVEL + "</font>.<br1><button value=\"Give me 1 month PREMIUM\" action=\"bypass -h Quest PremiumServiceNPC add_premium_1\" width=264 height=24 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"/><br1><button value=\"Give me 2 months PREMIUM\" action=\"bypass -h Quest PremiumServiceNPC add_premium_2\" width=264 height=24 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"/><br1><button value=\"Give me 3 months PREMIUM\" action=\"bypass -h Quest PremiumServiceNPC add_premium_3\" width=264 height=24 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"/><br1><br><br> Your Status:<br>Are you Premium user? <font color=LEVEL>%premiumacc%</font><br1>Use <font color=LEVEL>.premium</font> for more info about your acc.</center></body></html>";
			...
			case "add_premium_1":
				...
				else
				{
					...
					else
					{
						...
						{
							player.sendMessage("You need " + ITEM_NEED_FOR_1_MONTH + " special items.");
							player.broadcastPacket(new Earthquake(player.getX(), player.getY(), player.getZ(), 20, 10));
							player.broadcastPacket(new ExRedSky(10));
							player.broadcastPacket(new SocialAction(player, 11));
						}
			...
			case "add_premium_2":
				...
				else
				{
					...
					else
					{
						if (player.getInventory().getItemByItemId(BLUE_EVA) == null)
						{
							player.sendMessage("You need " + ITEM_NEED_FOR_2_MONTH + " special items.");
							player.broadcastPacket(new Earthquake(player.getX(), player.getY(), player.getZ(), 20, 10));
							player.broadcastPacket(new ExRedSky(10));
							player.broadcastPacket(new SocialAction(player, 11));
						}
			...
			case "add_premium_3":
				...
				else
				{
					...
					else
					{
						if (player.getInventory().getItemByItemId(BLUE_EVA) == null)
						{
							player.sendMessage("You need " + ITEM_NEED_FOR_3_MONTH + " special items.");
							player.broadcastPacket(new Earthquake(player.getX(), player.getY(), player.getZ(), 20, 10));
							player.broadcastPacket(new ExRedSky(10));
							player.broadcastPacket(new SocialAction(player, 11));
						}
