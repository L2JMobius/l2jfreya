package com.l2jserver.scripts.l2jpdt_npcs.SchemeBuffer;
...
public class NpcBuffer extends Quest
{
	...
	private String viewAllBuffs(String type, String typeName, String page)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (type.equals("set"))
			{
				querry = "SELECT * FROM npcbuffer_buff_list WHERE buffType IN (" + generateQuery(0, 0) + ") AND canUse=1";
			}
			else
			{
				querry = "SELECT * FROM npcbuffer_buff_list WHERE buffType=" + type;
			}
			
			try (PreparedStatement getBuffCount = con.prepareStatement(querry))
			{
				try (ResultSet rss = getBuffCount.executeQuery())
				{
					while (rss.next())
					{
						String name = SkillTable.getInstance().getInfo(rss.getInt("buffId"), rss.getInt("buffLevel")).getName();
	...
	private void heal(L2PcInstance player, boolean isPet)
	{
		...
		else if (((target = player.getSummon()) != null) || ((target = player.getPet()) != null))
		{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (DEBUG)
		{
			System.out.println(getName() + "#onAdvEvent('" + event + "'," + (npc == null ? "NULL" : npc.getNpcId() + npc.getName()) + "," + (player == null ? "NULL" : player.getName()) + ")");
		}
		...
		switch (eventParam0)
		{
			...
			case "delete":
				try (Connection con = L2DatabaseFactory.getInstance().getConnection())
				{
					try (PreparedStatement rem1 = con.prepareStatement("DELETE FROM npcbuffer_scheme_list WHERE id=? LIMIT 1"))
					{
						rem1.setString(1, eventParam1);
						rem1.executeUpdate();
					}
					try (PreparedStatement rem2 = con.prepareStatement("DELETE FROM npcbuffer_scheme_contents WHERE scheme_id=?"))
					{
						rem2.setString(1, eventParam1);
						rem2.executeUpdate();
					}
				}
				catch (SQLException e)
			...
			case "heal":
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					if (getSummonbuff)
					{
						if ((player.getSummon() != null) || (player.getPet() != null))
						{
				...
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					for (int i = 0; i < buffs.size(); ++i)
					{
						...
						else
						{
							...
							{
								SkillTable.getInstance().getInfo(buffs.get(i), levels.get(i)).getEffects(player.getPet(), player.getPet());
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
				...
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					else
					{
						...
						else
						{
							...
							{
								SkillTable.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2)).getEffects(player.getPet(), player.getPet());
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
