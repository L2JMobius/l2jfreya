 ...
 */
package com.l2jserver.scripts.custom.RaidbossInfo;
...
public final class RaidbossInfo extends L2AttackableAIScript
{
	...
	public RaidbossInfo()
	{
		...
		StartNpcs(NPC);
		TalkNpcs(NPC);
		RAIDS.addAll(NpcTable.getInstance().getByFilter(npc -> npc.getType().equalsIgnoreCase("l2raidboss")).stream().map(npc -> npc.getNpcId()).collect(Collectors.toList()));
