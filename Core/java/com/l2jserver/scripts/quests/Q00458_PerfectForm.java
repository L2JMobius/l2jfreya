...
public class Q00458_PerfectForm extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		QuestState st = player.getQuestState(Q00458_PerfectForm.class.getSimpleName());
		...
		if (npc.getNpcId() == _kelia)
		{
			...
			else if (event.equalsIgnoreCase("32768-16.htm"))
			{
				...
				{
					htmltext = getHtm(st.getPlayer(), "32768-16c.htm");
					htmltext = htmltext.replace("%overhits%", "" + mobsoverhitCount);
				}
				else if ((mobsoverhitCount >= 7) && (mobsoverhitCount <= 19))
				{
					htmltext = getHtm(st.getPlayer(), "32768-16b.htm");
					htmltext = htmltext.replace("%overhits%", "" + mobsoverhitCount);
				}
				else if (mobsoverhitCount >= 20)
				{
					htmltext = getHtm(st.getPlayer(), "32768-16a.htm");
					htmltext = htmltext.replace("%overhits%", "" + mobsoverhitCount);
				}
			}
